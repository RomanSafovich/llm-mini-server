...
async def ingest_file(doc_id: str = Form(...), file: UploadFile = File(...)):
    ...
    req = IngestTextRequest(doc_id=doc_id, text=extracted.text, metadata=extracted.metadata)
    return run_ingest(req, store=store, embedder=embedder)
...
def chat_rag(req: ChatRagRequest):
    return run_chat_rag(
        ...
        tokenizer=llm_manager.tokenizer,
    )
...
def chat_llm(prompt: Prompt):
    return {
        "answer": generate_text(
            prompt.prompt, tokenizer=llm_manager.tokenizer, model=llm_manager.model
        )
    }
...
def compare_documents_agent(req: CompareDocumentsAgentRequest):
    return run_compare_documents_agent(
        req,
        store=store,
        embedder=embedder,
        model=llm_manager.model,
