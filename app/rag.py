...
def run_chat_rag(req: ChatRagRequest, store, embedder, model, tokenizer) -> ChatRagResponse:
    ...
    hits = retrieve_unique_hits(
        question, effective_top_k=effective_top_k, store=store, embedder=embedder, doc_id=req.doc_id
    )
    ...
    logger.info(
        "top1_score=%s, top2_score=%s, margin=%s, top_k_scores=%s",
        top1_score,
        top2_score,
        margin,
        top_k_scores,
    ...
    if top1_score < settings.score_threshold:
        ...
        logger.info(
            "CTX: effective_top_k=%s, used_chunks=%s, used_chars=%s",
            effective_top_k,
            used_chunks,
            used_chars,
        )
        ...
        return ChatRagResponse(answer=ans, sources=sources_out, retrieved_count=len(used_hits))
